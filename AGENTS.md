...
| `AnalyseTool.Core` | Sdk | Platform: `CommandQueue` (single entry point for ALL transports), `CommandDispatcher`, extension loader (collectible ALC, type-identity sharing), Roslyn scripting, `CoreServices`, the model index (`Common/Index`: SQLite copy of each open model via the Windows `winsqlite3.dll`, fed by `DocumentChanged`, queried by `QueryModelIndex` off the Revit thread — see `LLM Wiki/wiki/analyses/model-index-plan.md`). **Headless — no WPF, no dialogs; errors go to Serilog + `ExtensionDiagnostics`.** |
