...
| `AnalyseTool.Cli` | Core, Sdk | Command line into Core, shipped inside the plugin folder: `policy show/validate/keygen/sign/verify`, `ext list/validate/install`, `org status/join/leave`, `diag collect`, `update wait-and-install` (per-user self-update helper). Never loads Revit; another *caller* of Core like `Mcp.Bridge`. |
