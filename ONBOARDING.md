...
### 4.7 Returning a picture to an AI client

A command whose result has a top-level `image` property of the shape
...
and structured content, so the tokens are not spent twice. The built-in `GetPdfPageAsImage` uses this,
and any extension command can too: give its result type such a property.
