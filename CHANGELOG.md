...
- 🧊 **The window explains itself while Revit is busy (#102).** A long Revit operation holds the UI thread that every WebView message is delivered on — so during a six-minute `ExecuteRevitCode` the status poll never reached the host, and the bar whose one job is to say "Revit is busy" was the thing that went silent. The page now watches its own unanswered calls on a plain timer the host cannot block: after three seconds without an answer it shows an amber "Revit is busy — it is running … on its main thread, this window will answer when it finishes" with a running count, and the last known command name if there was one. What the host cannot tell the page, the page tells itself.
