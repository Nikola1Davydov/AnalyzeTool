...
- 🕹️ **Minecraft for humans** — `samples/minecraft-panel` is a dockable block-building panel, shipped as a regular UI-only extension (plugin.json + one HTML page — also the minimal reference for UI-only extensions): pick a block from the palette (or type a custom one), hit **Build** and place blocks by clicking in the model — clicking an existing block stacks on top, Esc ends the session, and every block is its own undo step. Region fill/clear included. The interactive `McBuildInteractive` command ships with the host.
