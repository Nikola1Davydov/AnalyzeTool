...
- 🕹️ **Minecraft for humans** — `samples/minecraft-panel` is a first-person voxel GAME in an AnalyseTool window (three.js, UI-only extension), with the Revit model as the save file: walk/sprint/jump/fly, break blocks with the left mouse button, place them with the right — and every change is created or deleted in the Revit model live; reopening the game loads the world back from the model (`McGetBlocks`). Pixel-art textures are generated procedurally (drop PNGs into `textures/` to override). Enter your own cube family's name (e.g. `McCube`) and the game places THAT family — per block type it duplicates a family TYPE and sets its material parameter, so your family's Material drives the look (`familyName` option on the Mc* commands). The click-in-Revit `McBuildInteractive` command ships with the host too.
