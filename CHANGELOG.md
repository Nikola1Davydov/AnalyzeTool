...
- 🕹️ **Minecraft for humans** — a new **Minecraft** ribbon button opens a dockable panel: pick a block from the palette (or type a custom one), hit **Build** and place blocks by clicking in the model — clicking an existing block stacks on top, Esc ends the session, and every block is its own undo step. The panel also fills or clears whole regions (solid or hollow shell) without an AI.
