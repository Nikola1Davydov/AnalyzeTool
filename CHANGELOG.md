...
- 🤖 **The model bot — headroom sweep** — `BotScanHeadroom` walks a grid of standing points through every room, looks straight up and reports the rooms where the clear height drops below 2.1 m (yours to change), naming the duct, beam or tray that causes it and the rooms one element spoils at once. These defects intersect nothing, so clash detection is blind to them — the only way to see them is to ask the question from where a person stands.
- 👁️ **The bot's senses, for you and for the AI** — `BotLookAround` describes a point from the inside (which room, how far down to the floor and up to the ceiling, a fan of horizontal sight lines), and `BotRaycast` answers a single question: what is the first thing between here and there. Both are exposed over MCP, so a connected agent finally reasons about a *place* and not only about a list of elements. Everything reads: the raycasts run in a throwaway 3D view that is rolled back, so nothing is left in the model and no undo entry is created.
