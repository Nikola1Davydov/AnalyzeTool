...
#97 и #98 закрыты. Вторым заходом (после перезапуска Revit) подтверждён переход на
`builtInCategory`: `GetModelOverview.categories[]` несёт `OST_*` рядом с локализованным
именем, `GetCategoryParameters` принимает `builtInCategory: "OST_Walls"` и отвечает
объектом с обоими именами, а на `category: "Wand"` — `error` и `didYouMean: ["Wände"]`
