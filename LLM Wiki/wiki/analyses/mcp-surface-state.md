...
полем. **Не подтверждено вживую** (Revit был закрыт); подтверждение — следующий прогон.
...
отдаёт его тип и сообщение, пишет всю цепочку с именем команды и payload'ом в лог. Улика,
что это было нужно: сегодняшний лог хоста содержит четыре строки «GetElements invoked via
mcp» и **ни одной** об ошибке.
