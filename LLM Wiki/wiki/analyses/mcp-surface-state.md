...
## Дефект, которого нет в списке: агент слеп
...
## Тестировать по-прежнему негде

К [#105](https://github.com/Nikola1Davydov/AnalyzeTool/issues/105) («тест, который
реально зовёт каждую MCP-команду»): из code sweep в комментарии к
[#70](https://github.com/Nikola1Davydov/AnalyzeTool/issues/70) от 01.08.2026 —
`AnalyseTool.Test` это один `UnitTest1.cs`, и он ссылается на `AnalyseTool.App`, то есть
тянет за собой Revit. **Проверено по коду 2026-08-31: так и есть.** «Юнит-тесты против
поддельного диспетчера, без Revit» сегодня физически негде разместить, и это предусловие
самого дешёвого противодействия вытеснению.
