...
export interface BulkRenameItem {
  ...
  /** Uniqueness scope: names collide only within the same scope (family names are global — omit;
   *  type names are unique per family — pass the family id). */
  scope?: string;
}
...
const props = withDefaults(
  defineProps<{
    ...
    /** Every name currently taken (collision check), including the items themselves, with its scope. */
    taken: TakenName[];
    ...
    title?: string;
    /** Hide the built-in AI block (when a generator slot fully replaces it). NOTE: needs an explicit
     *  default — Vue casts an ABSENT boolean prop to false, which would hide the block everywhere. */
    showAi?: boolean;
    /** Externally computed names (e.g. from a naming rule) — filled into the rows when set. */
    names?: Record<number, string> | null;
  }>(),
  { showAi: true, title: undefined, names: null },
);
...
// Names computed outside (naming rule engine) — fill them in whenever the parent provides a batch.
watch(
  () => props.names,
  (names) => {
    if (!names) return;
...
const conflicts = computed<Set<number>>(() => {
  ...
  // \0 separates scope from name so that ("ab", "c") and ("a", "bc") can't collide. Written as an
  // escape rather than a literal NUL byte: a raw NUL makes git, grep and `file` treat the source as
  // binary (it did, until this port).
  const keyOf = (scope: string | undefined, name: string) => `${scope ?? ""}\0${name.toLowerCase()}`;

  // Names taken outside this batch (the batch's own old names are being renamed away).
  const taken = new Set(props.taken.map((t) => keyOf(t.scope, t.name)));
  for (const r of rows.value) taken.delete(keyOf(r.scope, r.oldName));

  const seen = new Map<string, number>(); // scoped new name -> first row id
  for (const r of rows.value) {
    if (!r.include) continue;
    const name = r.newName.trim();
    if (!name || name.toLowerCase() === r.oldName.trim().toLowerCase()) continue;
    const key = keyOf(r.scope, name);
    if (taken.has(key)) bad.add(r.id);
...
<template>
  <Dialog
    ...
    :header="title ?? 'Rename with AI'"
    :style="{ width: 'min(46rem, 95vw)' }"
  >
    <div class="flex flex-col gap-3">
      <!-- Optional external generator (e.g. a naming rule) provided by the parent -->
      <slot name="generator" />

      <!-- Instruction + generate -->
      <div v-if="showAi" class="rounded-lg border border-surface-200 bg-surface-50 p-3 flex flex-col gap-2">
        <AiModelIndicator />
        <textarea
          v-model="prompt"
          rows="2"
